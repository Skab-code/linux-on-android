...
MIT — see [LICENSE](LICENSE).#!/usr/bin/env bash
...
local RV="$?"
printf "\\e[1;48;5;138m %s" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} NOTICE:  Generated script signal received ${RV:-UNKNOWN} near or at line number ${1:-UNKNOWN} by '${2:-UNKNOWNCOMMAND}'!  "
...
if [[ "$RV" = 0 ]]
then
printf "\\e[0;32mCommand \\e[1;32m'%s' \\e[0;32mversion %s\\e[1;34m: \\e[1;32m%s\\n" "${STRNRG:-}" "${VERSIONID:-}" "DONE ðŸ "
printf "\033]2;%s\\007" "${STRNRG:-}:  DONE ðŸ "
else
printf "\\e[0;32mCommand \\e[1;32m'%s' \\e[0;32mversion %s\\e[1;34m: \\e[1;32m%s\\n" "${STRNRG:-}" "${VERSIONID:-}" "[Exit Signal $RV] DONE ðŸ "
printf "\033]2;%s\\007" "${STRNRG:-} [Exit Signal $RV]:  DONE ðŸ "
fi
...
local RV="$?"
printf "\\e[1;48;5;138m %s" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} HANG UP:  Generated signal received ${RV:-UNKNOWN} near or at line number ${1:-UNKNOWN} by '${2:-UNKNOWNCOMMAND}'!  "
exit "$RV"
}
_STRPNTRT_() { # run on signal
local RV="$?"
printf "\\e[1;48;5;138m%s" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} SIGNAL:  Generated signal received ${RV:-UNKNOWN} near or at line number ${1:-UNKNOWN} by '${2:-UNKNOWNCOMMAND}'!  "
exit "$RV"
}
_STRPQUIT_() { # run on quit
local RV="$?"
printf "\\e[1;48;5;138m %s" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} QUIT:  Quit signal received ${RV:-UNKNOWN} near or at line number ${1:-UNKNOWN} by '${2:-UNKNOWNCOMMAND}'!  "
exit "$RV"
}
_STRPTERM_() { # run on terminate
local RV="$?"
printf "\\e[1;48;5;138m %s" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} TERMINATE:  Generated signal received ${RV:-UNKNOWN} near or at line number ${1:-UNKNOWN} by '${2:-UNKNOWNCOMMAND}'!  "
exit "$RV"
}
...
if [ "$EUID" = 0 ] || [ "$UID" = 0 ]
then
printf "\\e[1;48;5;168m%s\\e[0m  " "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} SIGNAL:  Please do not use the root login for PRoot:  EXITING..." && exit
fi
if [ -w /root ]
then
printf "\\e[1;48;5;138m%s\\e[0m  " "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} SIGNAL:  Please run '${0##*/}' and 'bash ${0##*/}' from the BASH shell in native Termux:  EXITING..." && exit
fi
...
printf "\\e[0;34m%s \\e[1;34m%s \\e[1;32m%s\\n" " ðŸ•› = ðŸ•›" "TermuxArch version $VERSIONID integrity:" "OK"
exit 123
else
printf "\\n\\e[0;34m%s \\e[1;34m%s \\e[1;32m%s\\n" " ðŸ•› > ðŸ•œ" "TermuxArch version $VERSIONID integrity:" "OK"
_CHKSELF_
_COREFILESLOAD_
fi
else
...
_CHKDWN_() {
sha512sum -c --quiet setupTermuxArch.sha512 && printf "\\e[0;34m%s\\e[1;34m%s\\e[1;32m%s\\n" " ðŸ•› > ðŸ• " "TermuxArch download: " "OK" || _PRINTSHA512SYSCHKER_
TMPCMD="bsdtar -x -p -f setupTermuxArch.tar.gz" && $TMPCMD || printf "\\e[1;48;5;138m%s\\e[0m  " "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} SIGNAL $TMPCMD:  CONTINUING..."
}
...
printf "\\n\\e[0;34m ðŸ•› > ðŸ•§ \\e[1;34mPrerequisites: \\e[1;32mOK  \\e[1;34mDownloading TermuxArch...\\n\\n\\e[0;32m"
}
_DEPENDSBLOCK_() {
...
_INTRO_() {
printf "\033]2;%s\007" "bash $STRNRG ðŸ“²"
_SETROOT_EXCEPTION_
_INSTLLDIRCHK_
...
OPT=BLOOM
printf "\033]2;%s\007" "bash ${0##*/} bloom ðŸ“²"
...
_INTROSYSINFO_() {
printf "\033]2;%s\007" "bash ${0##*/} sysinfo ðŸ“²"
_SETROOT_EXCEPTION_
...
then
printf "\\n\\e[0;33m%s\\e[1;33m%s\\e[0;33m.\\n\\n" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} NOTICE!  " "The root directory structure is of ~/${INSTALLDIR##*/} seems to be incorrect; Cannot continue '$STRNRG'!  This command '${0##*/} help' has more information"
if [ -d "$INSTALLDIR"/tmp ]
...
_INTROREFRESH_() {
printf '\033]2;  bash setupTermuxArch refresh ðŸ“² \007'
if [ "${OPT:-}" = FORCE ]
then
_DODIRCHK_
else
...
then
printf "\\n\\e[0;33m%s\\e[1;33m%s\\e[0;33m.\\n\\n" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} NOTICE!  " "The root directory structure of ~/${INSTALLDIR##*/} appears correct;  Cannot continue '$STRNRG' to install Arch Linux in TermuxArch PRoot QEMU!  Commands '${0##*/} h[e[lp]]' have more information"
exit 205
fi
}
_LOADCONF_() {
...
_MANUAL_() {
printf '\033]2; bash setupTermuxArch manual ðŸ“² \007'
if [[ -f "${WDIR}setupTermuxArchConfigs.bash" ]]
then
...
_PRINTCONFLOADED_() {
printf "\\n\\e[0;34m%s \\e[1;34m%s \\e[0;32m%s\\e[1;32m%s \\e[1;34m%s \\e[1;32m%s\\n" " ðŸ•› > ðŸ•‘" "TermuxArch configuration" "$WDIR" "setupTermuxArchConfigs.bash" "loaded:" "OK"
}
_PRINTSHA512SYSCHKER_() {
printf "\\n\\e[07;1m\\e[31;1m\\n%s \\e[34;1m\\e[30;1m%s \\n\\e[0;0m\\n" " ðŸ”† ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} NOTICE sha512sum mismatch!  Setup initialization mismatch!  Is your wireless on?" "  Try again, initialization was not successful this time.  Wait a little while.  Then run the command 'bash $STRNRG' again..."
...
if [[ "$CPUABI" == "$1" ]]
then
printf "\\e[1;33m %s\\e[0;33m  %s\\e[1;31m  %s  " "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^}" "QEMU NOTICE!  Machine architecture is $CPUABI.  Please choose a different computer architecture." "Exiting..."
exit 189
fi
}
_INSTLLDIRCHK_
...
_RMARCHQ_() {
printf "\\n\\e[0;33m %s \\e[1;33m%s \\e[0;33m%s\\n\\n\\e[1;30m%s\\n" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^}" "DIRECTORY NOTICE!  ~/${INSTALLDIR##*/}/" "directory detected." "Purge '$INSTALLDIR' as requested?"
if [[ -z "${PURGELCR:-}" ]]
then
PURGEMETHOD="quick "
else
...
_COMMANDGNE_() { printf "\\n\\e[1;48;5;138m%s\\n\\n" "ï¼´ï½…ï½’ï½ï½•ï½˜ï¼¡ï½’ï½ƒï½ˆ ${PGNM^^} NOTICE:  Run '${0##*/}' and 'bash ${0##*/}' from the native BASH shell in Termux:  EXITING..." && exit 126 ; }
...
## Very many hardy thank yous to contributors who are helping and have worked very hard for many long years, some for more, and to those who took mere minutes from their valuable effort with time in order to make this open source resource much better for all of us!  Please enjoy using TermuxArch in PRoot QEMU in Android, Chromebook, Amazon Fire OS and Windows on smartphone, tablet, wearable and similar:  setupTermuxArch #!/usr/bin/env bash
## Copyright 2017-2022 (c) by SDRausty, all rights reserved, see LICENSE
## hosting termuxarch.github.io/TermuxArch courtesy pages.github.com
## termuxarch.github.io/TermuxArch/CONTRIBUTORS thank you for your help!
################################################################################
